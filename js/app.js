 document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupSearchEvents() {
        elements.searchBtn.addEventListener('click', async () => {
            await ui.performSearch(elements.searchInput.value);
        });
        ...
        elements.searchInput.addEventListener('keydown', async (e) => {
            ...
            if (e.key === 'Escape') {
                ...
                if (ui.currentCategory) {
                    await ui.loadLinks(ui.currentCategory);
                }
            }
        });
    }
    ...
    function setupAdminEvents() {
        ...
        elements.adminTabs.forEach(tab => {
            tab.addEventListener('click', async () => {
                ...
                if (tabId === 'info') {
                    ...
                    await ui.loadInfoIntoForm(infoNum);
                    await ui.updateInfoActionButtons(infoNum);
                }
        ...
        elements.infoSelect.addEventListener('change', async () => {
            const infoNum = elements.infoSelect.value;
            await ui.loadInfoIntoForm(infoNum);
            await ui.updateInfoActionButtons(infoNum);
        });
        ...
        elements.addLinkBtn.addEventListener('click', async () => {
            ...
            try {
                await storage.addLink(category, {
                    nome: name,
                    url: normalizedUrl,
                    desc: desc
                });
        ...
        elements.saveInfoBtn.addEventListener('click', async () => {
            ...
            try {
                const infoData = await storage.getInfo();
                ...
                await storage.saveInfo(infoData);
                await ui.loadInfoFromStorage();
        ...
        elements.clearStorageBtn.addEventListener('click', () => {
            ui.showConfirmModal(
                'Tem certeza que deseja limpar TODOS os dados? Links personalizados, informativos e configurações de PIN serão perdidos e restaurados aos padrões.'
            );
            ...
            appState.confirmCallback = async () => {
                try {
                    ...
                    await ui.loadInfoFromStorage();
                    await ui.loadInfoIntoForm(elements.infoSelect.value);
                    await ui.updateInfoActionButtons(elements.infoSelect.value);
                    ...
                    ui.showAdminMessage(error.message || 'Erro ao restaurar sistema.', 'error');
                    ui.showToast(error.message || 'Erro ao restaurar sistema.', 'error');
                }
            };
        });
        ...
        elements.adminLinksList.addEventListener('click', (e) => {
            ...
            ui.showConfirmModal('Excluir este link?');

            appState.confirmCallback = async () => {
                try {
                    await storage.deleteLink(category, index);
                    await ui.loadAdminLinks();
                    ...
                    ui.showAdminMessage(error.message || 'Erro ao excluir link.', 'error');
                    ui.showToast(error.message || 'Erro ao excluir link.', 'error');
                }
            };
         });
        ...
        if (resetCurrentInfoBtn) {
            resetCurrentInfoBtn.addEventListener('click', async () => {
                const infoNum = elements.infoSelect.value;

                if (!(await ui.hasCustomizedInfo(infoNum))) {
                ...
                appState.confirmCallback = async () => {
                    try {
                        await ui.restoreCurrentInfoToDefault(infoNum);
                        ...
                        ui.showAdminMessage(error.message || 'Erro ao restaurar painel.', 'error');
                        ui.showToast(error.message || 'Erro ao restaurar painel.', 'error');
                    }
                };
            });
        }
    ...
    init().catch((error) => {
        ...
        ui.showToast(error.message || 'Erro ao iniciar o sistema', 'error');
    });
});
