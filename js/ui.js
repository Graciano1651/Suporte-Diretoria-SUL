class UIManager {  
    constructor(storage) {  
        ...
        this.defaultInfoHTML = {}; // Cache do conteúdo padrão  
    }  
  
    // ====== Gerenciamento individual de informativos ======
restoreCurrentInfoToDefault(infoNum) {
    const infoData = this.storage.getInfo();

    // Remove apenas o painel atual do storage
    delete infoData[infoNum];
    this.storage.saveInfo(infoData);

    // Recarrega tudo usando a lógica já existente
    this.loadInfoFromStorage();

    // Recarrega o formulário do painel selecionado
...
updateInfoActionButtons(infoNum) {
    ...
    // ====== Cache do conteúdo padrão (com trava) ======  
    cacheDefaultInfoContent() {  
        // Evita sobrescrever o cache se o método for chamado de novo  
    ...
    loadLinks(category) {  
        ...
        if (!categoryLinks.length) {  
            linksGrid.appendChild(this.buildEmptyState(  
                'fas fa-folder-open',  
                'Nenhum link disponível para esta categoria.',  
                'Adicione links através do painel administrativo.'  
            ));  
            return;  
        }  
  
        categoryLinks.forEach(link => {  
            linksGrid.appendChild(this.buildLinkItem(link));  
        });  
    }  
    ...
    buildLinkItem(link) {  
        ...
        if (href) {  
            ...
            a.addEventListener('click', (e) => {  
                e.preventDefault();  
                this.showToast('URL inválida ou não configurada', 'error');  
            });  
            a.title = 'URL inválida';  
            a.classList.add('invalid-link');  
        }  
        ...
        if (!href) {  
            ...
            const warningText = document.createTextNode(' URL precisa de correção');  
    ...
    performSearch(query) {  
        ...
        if (!query) {  
            if (this.currentCategory) this.loadLinks(this.currentCategory);  
            return;  
        }  
        ...
        results.forEach(link => {  
            linksGrid.appendChild(this.buildSearchResultItem(link));  
        });  
    }  
    ...
    buildSearchResultItem(link) {  
        ...
        icon.className = 'fas fa-external-link-alt';  
        a.appendChild(icon);  
        a.appendChild(document.createTextNode(link.nome || 'Link'));  
  
        const badge = document.createElement('span');  
        badge.className = 'category-badge';  
        badge.textContent = this.categoryLabel(link.category);  
    ...
    showInfo(infoNum) {  
        ...
        infoDots.forEach(dot => {  
            dot.classList.toggle('active', Number(dot.dataset.info) === infoNum);  
        });  
    }  
    ...
    // ====== CARREGAMENTO DE INFORMATIVOS COM FALLBACK PARA CONTEÚDO PADRÃO ======  
    loadInfoFromStorage() {  
        const infoData = this.storage.getInfo();  
        ...
        for (let i = 1; i <= 4; i++) {  
            ...
            // Se não existe nada salvo, restaura o HTML padrão original  
            if (!info || (!info.title && !info.content && !info.imageData)) {  
                infoText.classList.remove('has-image-only', 'has-image-with-text');  
  
                // Restaura o conteúdo padrão do cache  
                if (this.defaultInfoHTML[i] !== undefined) {  
                    infoText.innerHTML = this.defaultInfoHTML[i];  
                }  
  
                continue;  
            }  
  
            // Se existe conteúdo salvo, limpa e renderiza com os dados do storage  
            ...
            // Aplica a classe correta baseada no conteúdo  
            if (hasImage && !hasText) {  
                infoText.classList.add('has-image-only');  
            } else if (hasImage && hasText) {  
                infoText.classList.add('has-image-with-text');  
            }  
  
            // Renderiza imagem se existir  
            if (hasImage) {  
                const imageContainer = document.createElement('div');  
                imageContainer.className = 'info-image-container';  
  
                const img = document.createElement('img');  
                img.src = info.imageData;  
                img.alt = hasTitle ? info.title : 'Imagem do informativo';  
                ...
                infoText.appendChild(imageContainer);  
            }  
    ...
    setupImageUpload() {  
        ...
        infoImage.addEventListener('change', (e) => {  
            ...
            if (file.size > 2 * 1024 * 1024) {  
                this.showToast('Imagem muito grande. Máximo 2MB.', 'error');  
                infoImage.value = '';  
                this.currentImageFile = null;  
                return;  
            }  
            ...
            reader.onload = (ev) => {  
                imagePreview.src = String(ev.target.result || '');  
    ...
    loadInfoIntoForm(infoNum) {  
        ...
        if (info.imageData) {  
            imagePreview.src = info.imageData;  
            imagePreviewContainer.style.display = 'block';  
        } else {  
            imagePreview.src = '#';  
            imagePreviewContainer.style.display = 'none';  
    ...
    loadAdminLinks() {  
        ...
        categories.forEach(category => {  
            ...
            h5.textContent = category.toUpperCase();  
            title.appendChild(h5);  
            adminLinksList.appendChild(title);  
  
            categoryLinks.forEach((link, index) => {  
                adminLinksList.appendChild(this.buildAdminLinkItem(category, link, index));  
                totalRendered++;  
            });  
        });  
  
        if (!totalRendered) {  
            adminLinksList.appendChild(this.buildEmptyState(  
                'fas fa-folder-open',  
    ...
    buildAdminLinkItem(category, link, index) {  
        ...
        const h5 = document.createElement('h5');  
        h5.textContent = link.nome || 'Link';  
  
        const pUrl = document.createElement('p');  
        pUrl.textContent = link.url || '';  
        ...
        if (!this.storage.validateUrl(link.url)) {  
            ...
            const warningText = document.createTextNode(' URL inválida');  
    ...
    hideAuthModal() {  
        const { authModal } = this.getElements();  
        authModal.classList.remove('active');  
    }  
  
    showConfirmModal(message) {  
        const { confirmModal, modalMessage } = this.getElements();  
        modalMessage.textContent = message;  
        confirmModal.classList.add('active');  
    }  
  
    hideConfirmModal() {  
        const { confirmModal } = this.getElements();  
    ...
    hidePageModal() {  
        const { pageModal } = this.getElements();  
