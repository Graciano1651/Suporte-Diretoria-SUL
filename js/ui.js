class UIManager {
    ...
    async restoreCurrentInfoToDefault(infoNum) {
        const infoData = await this.storage.getInfo();
        delete infoData[infoNum];
        await this.storage.saveInfo(infoData);
        await this.loadInfoFromStorage();
        await this.loadInfoIntoForm(infoNum);
    }

    async hasCustomizedInfo(infoNum) {
        const infoData = await this.storage.getInfo();
        return !!infoData[infoNum];
    }

    async updateInfoActionButtons(infoNum) {
        const resetBtn = document.getElementById('reset-current-info');
        if (!resetBtn) return;

        resetBtn.disabled = !(await this.hasCustomizedInfo(infoNum));
    }
    ...
    async loadLinks(category) {
        ...
        const allLinks = await this.storage.getLinks();
    ...
    buildLinkItem(link) {
        ...
        if (!href) {
            ...
            warning.appendChild(document.createTextNode(' URL precisa de correção'));
    ...
    async performSearch(query) {
        ...
        if (!query) {
            if (this.currentCategory) {
                await this.loadLinks(this.currentCategory);
            }
            return;
        }

        this.clearElement(linksGrid);
        const results = await this.storage.searchLinks(query);
    ...
    async loadInfoIntoForm(infoNum) {
        const {
            infoTitle,
            infoContent,
            infoImage,
    ...
    async loadAdminLinks() {
        ...
        const links = await this.storage.getLinks();
        ...
        if (!totalRendered) {
            adminLinksList.appendChild(
                this.buildEmptyState('fas fa-folder-open', 'Nenhum link cadastrado.', '')
            );
        }
    }
    ...
    buildAdminLinkItem(category, link, index) {
        ...
        if (!this.storage.validateUrl(link.url)) {
            ...
            warning.appendChild(warningIcon);
            warning.appendChild(document.createTextNode(' URL inválida'));
    ...
    hideAuthModal() {
        this.getElements().authModal.classList.remove('active');
    }
    ...
    hideConfirmModal() {
        this.getElements().confirmModal.classList.remove('active');
    }
    ...
    hidePageModal() {
        this.getElements().pageModal.classList.remove('active');
    }
